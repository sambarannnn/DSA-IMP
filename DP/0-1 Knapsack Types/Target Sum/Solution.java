class Solution {
    public int findTargetSumWays(int[] nums, int target) {
        ...
        int s1 = (sum + target ) / 2;
        return subsetsWithGivenSum(nums, nums.length, s1) * (int)Math.pow(2, zeroCount); //make permutations with number of zeroes
    }
    private int subsetsWithGivenSum(int[] nums, int n, int sum) {
